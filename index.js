...
function ensureDefault(key, value) {
  if (dbB.get(key) === null || dbB.get(key) === undefined) dbB.set(key, value);
}
ensureDefault("cor", process.env.COR_PADRAO || "#0073ff");
ensureDefault("imagem", process.env.IMAGEM_PADRAO || "");
ensureDefault("categoria", process.env.CATEGORIA_CARRINHO || "");
ensureDefault("logs", process.env.CANAL_LOGS || "");
ensureDefault("acesstoken", process.env.MP_ACCESS_TOKEN || "");
ensureDefault("nomebot", config.nomebot);
