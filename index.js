...
function getCfg(key) {
  const envMap = {
    ...
    mpAccessToken: 'MP_ACCESS_TOKEN', vendaChannel: 'CANAL_VENDAS', verificationChannel: 'VERIFICACAO_CANAL',
...
function commandHelp() {
  return new MessageEmbed()
    ...
    .setDescription([
      ...
      '`configbot` - Configura nome, cargo, Mercado Pago e cor.',
      ...
      '`status <id_pagamento>` - Consulta pagamento Mercado Pago.',
      '`taxa` - Mostra link de taxas Mercado Pago.',
...
register(['configbot'], async (message) => {
  ...
  const row = new MessageActionRow().addComponents(
    ...
    new MessageButton().setCustomId('cfgbot:mp').setLabel('Token MP').setStyle('SECONDARY'),
...
register(['status'], async (message, args) => {
  if (!isAdmin(message)) return message.channel.send({ embeds: [noPerm()] });
  const id = args[0];
  const token = getCfg('mpAccessToken');
  if (!id) return message.reply(`Use: \`${PREFIX}status id_pagamento\``);
  if (!token) return message.reply('❌ Token do Mercado Pago não configurado. Use `configbot`.');
  try {
    const doc = await axios.get(`https://api.mercadolibre.com/collections/notifications/${id}`, { headers: { Authorization: `Bearer ${token}` } });
    const approved = doc.data?.collection?.status === 'approved';
    const amount = doc.data?.collection?.transaction_amount || 0;
    return message.channel.send({ embeds: [embedBase('Status de Pagamento', `📌 | **Status:** ${approved ? 'Aprovado' : 'Cancelado/Reembolsado/Pendente'}\n💵 | **Preço:** R$${amount}\n🌎 | **ID:** ${id}`)] });
  } catch (err) {
    return message.channel.send('🚫 Aconteceu algum erro. Confirme se o ID está correto e se o token do Mercado Pago está configurado.');
  }
});

register(['taxa'], async (message) => {
  if (!isAdmin(message)) return message.channel.send({ embeds: [noPerm()] });
  return message.channel.send({ embeds: [new MessageEmbed().setTitle('Checkouts Mercado Pago').setDescription('[Mude as opções de taxa e liberação pelo Mercado Pago](https://www.mercadopago.com.br/costs-section/release-options/edit/merchant-services)').setColor(color())] });
});
...
async function handleCart(interaction) {
  ...
  const token = getCfg('mpAccessToken');
  if (!token) {
    const pixKey = getCfg('pixKey') || 'Configure PIX_KEY ou MP_ACCESS_TOKEN no Railway/configbot.';
    return interaction.channel.send({ embeds: [embedBase('Pagamento manual', `Total: **R$${total.toFixed(2)}**\nChave Pix: \`${pixKey}\`\nApós pagar, envie o comprovante para a staff.`)] });
  }
  mercadopago.configurations.setAccessToken(token);
  const paymentData = {
    transaction_amount: Number(total.toFixed(2)),
    description: `Pagamento - ${getCfg('nomebot')} | ${interaction.user.username}`,
    payment_method_id: 'pix',
    payer: { email: 'cliente@pagamento.com', first_name: interaction.user.username, last_name: 'Discord' }
  };
  ...
  const base64 = data.body?.point_of_interaction?.transaction_data?.qr_code_base64;
  if (!qrcode || !base64) return interaction.channel.send('❌ Não consegui gerar o Pix. Confira o token Mercado Pago.');
  const attachment = new MessageAttachment(Buffer.from(base64, 'base64'), 'payment.png');
  const payRow = new MessageActionRow().addComponents(
    new MessageButton().setCustomId('qr-disabled').setLabel('QR Code').setStyle('SECONDARY').setDisabled(true),
    new MessageButton().setCustomId(`cart:copy:${data.body.id}:0:none`).setLabel('Pix Copia e Cola').setStyle('SECONDARY'),
    new MessageButton().setCustomId('ticket:close').setLabel('Cancelar').setStyle('DANGER')
  );
  await interaction.channel.send({ embeds: [embedBase('Sistema de pagamento', `Produto: **${p.nome}**\nQuantidade: **${qtd}**\nTotal: **R$${total.toFixed(2)}**\nID da compra: \`${data.body.id}\``).setImage('attachment://payment.png')], files: [attachment], components: [payRow] });
  await interaction.channel.send({ content: `Pix copia e cola:\n\`\`\`${qrcode}\`\`\`` });
  const interval = setInterval(async () => {
    try {
      const doc = await axios.get(`https://api.mercadolibre.com/collections/notifications/${data.body.id}`, { headers: { Authorization: `Bearer ${token}` } });
      if (doc.data?.collection?.status === 'approved') {
        clearInterval(interval);
        finishPurchase(interaction, id, qtd, total, data.body.id, cupomCode);
      }
    } catch (err) {}
  }, 10000);
...
async function handleCfgBot(interaction) {
  ...
  const prompts = { nome: 'Envie o novo nome do bot.', cargo: 'Envie o ID do cargo cliente.', mp: 'Envie o token do Mercado Pago.', cor: 'Envie a cor HEX. Exemplo: #00ff00' };
  ...
  if (action === 'mp') setCfg('mpAccessToken', value);
