...
// Configuração SEM config.json: coloque estes valores nas Variables do Railway.
const config = {
...
if (!config.TOKEN) {
  console.error("❌ TOKEN não configurado. Adicione TOKEN nas Variables do Railway.");
  process.exit(1);
}
...
const dbB = new JsonDatabase({ databasePath:"./myJsonBotConfig.json" });
const dbm = new JsonDatabase({ databasePath:"./myJsonMoney.json"});
const dbv = new JsonDatabase({ databasePath:"./myJsonAvaliar.json" });
const dbc = new JsonDatabase({ databasePath:"./myJsonCupons.json" });
const { joinVoiceChannel } = require('@discordjs/voice');


// Valores padrão do banco JSON para evitar null em embeds/categorias/logs.
function ensureDefault(key, value) {
  // O wio.db nao aceita valores vazios como "", null ou undefined.
  // Por isso, sempre salvamos um valor real para o bot nao cair no Railway.
  if (value === undefined || value === null || value === "") return;

  const atual = dbB.get(key);
  if (atual === null || atual === undefined || atual === "") {
    dbB.set(key, value);
  }
}

ensureDefault("cor", process.env.COR_PADRAO || "#0073ff");
ensureDefault("imagem", process.env.IMAGEM_PADRAO || "https://cdn.discordapp.com/embed/avatars/0.png");
ensureDefault("categoria", process.env.CATEGORIA_CARRINHO || "0");
ensureDefault("logs", process.env.CANAL_LOGS || "0");
ensureDefault("acesstoken", process.env.MP_ACCESS_TOKEN || "SEM_TOKEN_MERCADOPAGO");
ensureDefault("nomebot", config.nomebot || "System Store");
...
moment.locale("pt-br");
client.login(config.TOKEN).catch(err => { console.error("❌ Erro ao logar no bot:", err); process.exit(1); });
...
client.on("ready", () => {
    if (!config.canalvoz) return console.log("ℹ️ CANAL_VOZ não configurado; pulando conexão em voz.");
    let channel = client.channels.cache.get(config.canalvoz);
    if (!channel) return console.log("⚠️ Canal de voz não encontrado; confira CANAL_VOZ.");
...
client.on('messageCreate', message => {
    ...
    // Sem pasta commands: os comandos externos foram desativados para rodar tudo só no index.js.
    if (command === "ping") return message.reply("🏓 Pong!");
    if (command === "help") return message.reply(`Comandos ativos: ${config.prefix}ping, ${config.prefix}help`);
});
...
.setColor(`${dbB.get(`cor`)}`)
                                           ...
                                           if (config.cargovip) membro.roles.add(config.cargovip).catch(console.error)
